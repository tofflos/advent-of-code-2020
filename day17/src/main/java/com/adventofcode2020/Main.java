...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        var bounds = new Bounds(0, input.get(0).length(), 0, input.size(), 0, 1);
        var current = new HashMap<Coordinate, Character>();

        for (int y = bounds.y1(); y < bounds.y2(); y++) {
            for (int x = bounds.x1(); x < bounds.x2(); x++) {
                current.put(new Coordinate(x, y, 0), input.get(y).charAt(x));
            }
        }
        ...
        System.out.println("Part 1: " + current.values().stream().filter(c -> c == '#').count());
    }
        ...
        for (int y = coordinate.y() - 1; y <= coordinate.y() + 1; y++) {
            for (int x = coordinate.x() - 1; x <= coordinate.x() + 1; x++) {
                for (int z = coordinate.z() - 1; z <= coordinate.z() + 1; z++) {
                    var neighbour = new Coordinate(x, y, z);
...
record Bounds(int x1, int x2, int y1, int y2, int z1, int z2) {
